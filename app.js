const APP_I18N = {
  ru: { login: 'Войти через Steam', logout: 'Выйти', navPlay: 'Играть', navLeaderboard: 'Лидерборд', navProfile: 'Профиль', navHome: 'Главная', brandSub: 'matchmaking app', queueBadgeIdle: 'Не в очереди', queueTitle: 'Найти матч', playCtaGuest: 'Сначала войди через Steam. Потом можно искать матч в 2x2 соло или вдвоём.', playCtaAuthed: 'Режим только 2x2: можно искать матч соло или вдвоём. Готовая пати из двух не разделяется, соло-игроку подбирается тиммейт.', joinQueue: 'НАЙТИ МАТЧ', cancel: 'ОТМЕНИТЬ', statusTitle: 'Статус', statusIdle: 'Ожидание', queueTimer: 'Поиск:', mode: 'Режим', party: 'Party', partyMembers: 'Состав party', partyInvites: 'Входящие инвайты', findPlayer: 'Найти игрока', history: 'История матчей', authConnected: 'Steam connected', guest: 'Гость', inviteToParty: 'Пригласить в party', invite: 'Пригласить' },
  en: { login: 'Sign in with Steam', logout: 'Sign out', navPlay: 'Play', navLeaderboard: 'Leaderboard', navProfile: 'Profile', navHome: 'Home', brandSub: 'matchmaking app', queueBadgeIdle: 'Not in queue', queueTitle: 'Find match', playCtaGuest: 'Sign in with Steam first. Then you can queue 2v2 solo or with a friend.', playCtaAuthed: '2v2 only: queue solo or as a duo. A ready two-player party stays together, and solo players get a teammate.', joinQueue: 'FIND MATCH', cancel: 'CANCEL', statusTitle: 'Status', statusIdle: 'Idle', queueTimer: 'Search:', mode: 'Mode', party: 'Party', partyMembers: 'Party roster', partyInvites: 'Incoming invites', findPlayer: 'Find player', history: 'Match history', authConnected: 'Steam connected', guest: 'Guest', inviteToParty: 'Invite to party', invite: 'Invite' }
};
...
function applyAppLang() {
  ...
  const map = { appLoginBtn:'login', appLogoutBtn:'logout', appNavPlay:'navPlay', appNavLeaderboard:'navLeaderboard', appNavProfile:'navProfile', appNavHome:'navHome', appMobileNavPlay:'navPlay', appMobileNavLeaderboard:'navLeaderboard', appMobileNavProfile:'navProfile', appMobileNavHome:'navHome', appBrandSub:'brandSub', queueBadge:'queueBadgeIdle', appQueueTitle:'queueTitle', joinQueueBtn:'joinQueue', cancelQueueBtn:'cancel', queueStatusTitle:'statusTitle', matchmakingState:'statusIdle', queueTimerLabel:'queueTimer', queueModeLabel:'mode', queuePartyLabel:'party', partyTitle:'party', partyMembersLabel:'partyMembers', partyInvitesLabel:'partyInvites', partySearchLabel:'findPlayer', historyTitle:'history' };
  ...
  root.innerHTML = RANK_TABLE.map((rank) => `
    <div class="rank-tooltip-row ${rank.key === activeRankKey ? 'active' : ''}">
      <div class="rank-tooltip-rank">
        <div>
          <div class="rank-tooltip-name">${getRankIconSrc(rank) ? `<img class="rank-medal-img" src="${esc(getRankIconSrc(rank))}" alt="" loading="lazy">` : ''}${esc(rank.name)}</div>
...
function formatMapName(map) {
  const raw = String(map || 'unknown').replace(/^de_/, '');
...
function getRankPillMarkup(rank, elo) {
  ...
  const icon = getRankIconSrc(info);
  const iconHtml = icon ? `<img class="rank-medal-img" src="${esc(icon)}" alt="" loading="lazy">` : '';
  return `<span class="rank-pill ${esc(info.color || 'silver')}" data-rank-key="${esc(info.key || '')}">${iconHtml}<span>${esc(info.name)}</span></span>`;
}
...
function renderAuth() {
  ...
  if (rankPill) { rankPill.innerHTML = `${getRankIconSrc(rank) ? `<img class="rank-medal-img" src="${esc(getRankIconSrc(rank))}" alt="" loading="lazy">` : ''}<span>${esc(rank.name)}</span>`; rankPill.className = `rank-pill ${rank.color || 'silver'}`; }
...
function renderQueue() {
  ...
  text('searchStateText', inQueue
    ? `Матчмейкер подбирает 2x2 игру. Сейчас ищут ${searchingPlayers} игроков.`
    : canQueue
...
function renderAcceptAndMapVoting(match) {
  ...
  if (canVoteMap) {
    ...
    html += `<div class="map-vote-grid">${state.mapPool.map((map) => `<button class="map-vote-card" data-map-vote="${esc(map)}"><span class="map-vote-bg"></span><span class="map-vote-title">${esc(formatMapName(map))}</span><span class="map-vote-sub">vote map</span></button>`).join('')}</div>`;
    html += '</div></div>';
...
function renderMatchRoomActions(match) {
  ...
  if (room.actions?.canVoteMap) {
    ...
    actions.push(`<div class="map-vote-grid">${state.mapPool.map((map) => `
      <button class="map-vote-card vote-btn" data-room-action="vote" data-map-name="${esc(map)}">
        <span class="map-vote-bg"></span>
        <span class="map-vote-title">${esc(formatMapName(map))}</span>
