...
async function refreshHealth() {
  try {
    ...
    $('backendText').textContent = 'Backend доступен и отвечает.';
  } catch (_) {
    $('backendBadge').textContent = 'OFFLINE';
    $('backendBadge').className = 'pill warn';
    $('backendText').textContent = 'Backend сейчас не ответил. Проверь Railway deploy, URL и CORS.';
  }
}
...
async function refreshConfig() {
  try {
    ...
    $('configBadge').textContent = cfg.matchmakingEnabled ? 'MATCHMAKING ON' : 'OFF';
    $('configBadge').className = `pill ${cfg.matchmakingEnabled ? 'ok' : 'warn'}`;
    $('configText').textContent = `${cfg.appName || 'TRUST'} • latest ${cfg.latestVersion || '—'} • режим ${cfg.mode || 'auto'}`;
  } catch (_) {
    $('configBadge').textContent = 'Ошибка';
    $('configBadge').className = 'pill warn';
    $('configText').textContent = 'Не удалось загрузить config.';
  }
}
