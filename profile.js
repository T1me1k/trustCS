const PROFILE_I18N = {
  ru: {
    ...
    detailedStats:'Расширенная статистика', statsUnavailable:'Расширенная статистика недоступна', wins:'Побед', avgDamage:'Средний урон', headshotRate:'Headshot rate', mvp:'MVP', kd:'K/D', currentStreak:'Текущая серия', bestStreak:'Лучшая серия', mapStats:'Статистика по картам'
  },
  en: {
    ...
    detailedStats:'Detailed statistics', statsUnavailable:'Detailed statistics unavailable', wins:'Wins', avgDamage:'Average damage', headshotRate:'Headshot rate', mvp:'MVP', kd:'K/D', currentStreak:'Current streak', bestStreak:'Best streak', mapStats:'Map statistics'
  }
};
...
function applyLang() {
  ...
  text('detailedStatsTitle', pt('detailedStats')); text('detailedStatsPill', pt('statsUnavailable')); text('mapStatsTitle', pt('mapStats'));
...
function renderHistory(items) {
  ...
  root.innerHTML = items.map((item) => {
    ...
    return `<${tag} class="profile-match-card" ${href ? `href="${href}" aria-label="Open match ${esc(matchIdOf(item))}"` : ''}>
      <div class="profile-match-main">
        <span class="pill ${resultClass(item.result)}">${item.result === 'win' ? 'WIN' : item.result === 'loss' ? 'LOSS' : '—'}</span>
        <div><strong>${esc(mapName(item.mapName))}</strong><div class="muted">${esc(formatDate(item.finishedAt || item.completedAt || item.startedAt))} • ${esc(formatDuration(item.durationSec || item.durationSeconds))}${kda ? ` • K/D/A ${esc(kda)}` : ''}${item.isMatchMvp ? ' • ★ MVP' : ''}</div></div>
      </div>
...
function renderDetailedStats(stats) {
  ...
  const cells = [[pt('matches'), data.matches ?? data.totalMatches], [pt('wins'), data.wins], [pt('winrate'), `${Number(data.winrate ?? data.winRate ?? 0)}%`], [pt('kd'), data.kd ?? calculateKd(data.kills, data.deaths)], [pt('avgDamage'), data.averageDamage ?? data.avgDamage], [pt('headshotRate'), `${Number(data.headshotRate ?? 0)}%`], [pt('mvp'), data.mvps ?? data.mvp], [pt('currentStreak'), data.currentStreak ?? data.currentWinStreak], [pt('bestStreak'), data.bestStreak ?? data.bestWinStreak]];
  root.innerHTML = cells.map(([label,value]) => `<div class="stat profile-stat"><span>${esc(label)}</span><strong>${esc(value ?? '—')}</strong></div>`).join('');
  const mapItems = data.maps || data.mapStats || [];
  maps.innerHTML = mapItems.length ? mapItems.map(m => `<div class="map-stat-row"><strong>${esc(mapName(m.map || m.mapName))}</strong><span>${esc(pt('matches'))}: ${esc(m.matches ?? 0)}</span><span>${esc(pt('wins'))}: ${esc(m.wins ?? 0)}</span><span>${esc(pt('winrate'))}: ${esc(m.winrate ?? m.winRate ?? 0)}%</span><span>${esc(pt('kd'))}: ${esc(m.averageKd ?? m.avgKd ?? '—')}</span></div>`).join('') : `<div class="empty">${esc(pt('noMatches'))}</div>`;
}
...
function renderProfile(profile, history, stats) {
  ...
  renderForm(profile.recentForm || []); renderHistory(history || []); renderDetailedStats(stats); text('profileInsight', profileInsight(profile));
}
async function boot() {
  applyLang();
  ...
  try {
    ...
    renderProfile(me.profile, hist.items || [], stats);
